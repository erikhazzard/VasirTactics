...
GAME_NAME.Views.Cell = (function(_super) {
  ...
  Cell.prototype.render = function(params) {
    ...
    this.tile_bg = this.tile_group.append('svg:image').attr('class', 'map_tile_image').attr('x', 0).attr('y', 0).attr('width', this.options.cellSize.width).attr('height', this.options.cellSize.height).attr('xlink:href', params.renderer.getCellImageSrc({
      cell: this.model
    }));
  ...
  Cell.prototype.mouseEnter = function() {
    return this.$el.attr('class', this.$el.attr('class') + ' map_tile_mouse_over');
  };

  Cell.prototype.mouseLeave = function() {
    return this.$el.attr('class', this.$el.attr('class').replace(/\ map_tile_mouse_over/gi, ''));
  };
